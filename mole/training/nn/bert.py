...
from mole.training.nn.disentangled_attention import DisentangledSelfAttention
    ...
    def __init__(self, config):
        ...
        if self.relative_attention:
            ...
            self.rel_embeddings = nn.Embedding(
                pos_ebd_size, config.hidden_size, padding_idx=0
            )
    ...
    def forward(self, context_token, labels=None):
        ...
        if labels is not None:
            if self.num_classes > 1:
                ...
                labels = labels.view(-1)[~labels.isnan()].to(logits)
                loss = self.loss_fn(logits, labels.long())
            else:
                logits = logits.view(-1)[~labels.isnan()]
                labels = labels.view(-1)[~labels.isnan()].to(logits)
